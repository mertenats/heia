...
static struct aci_state_t aci_state = {0};

// Temporary buffers for sending ACI commands 
static hal_aci_evt_t aci_data = {0};
...
/**************************************************************************/
void nRF8001Device::setACIcallback(aci_callback pACIEvent) 
{
  m_pACIEvent = pACIEvent;
}
  ...
  if (lib_aci_event_get(&aci_state, &aci_data))
  {
    ...
    switch (aci_evt->evt_opcode)
    {
        ...
        switch(aci_evt->params.device_started.device_mode)
        {
          ...
          case ACI_DEVICE_STANDBY:
          {
            ...
            if (m_szDeviceName[0] != 0x00)
            {
              ...
              lib_aci_set_local_data(&aci_state, 1 /*PIPE_GAP_DEVICE_NAME_SET */, (uint8_t *) &m_szDeviceName, strlen(m_szDeviceName));
            }
            ...
            if (m_pACIEvent != NULL) 
            {
              m_pACIEvent(ACI_EVT_DEVICE_STARTED);
            }
          }          
          break;
        }
      ...
      case ACI_EVT_DISCONNECTED:
      {
        ...
        if (m_pACIEvent)
        {
          m_pACIEvent(ACI_EVT_DISCONNECTED);
        }
