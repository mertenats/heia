...
static struct spi_ctrl spi2;


void touchscreen_init()
{
        /* initialize the spi interface */
        spi2 = spi_init(IMX27_SPI2, IMX27_SS0, 8000, 16);
...
struct touchscreen_position touchscreen_read_position()
{
        uint32_t d[3];
        uint32_t c = 0x8000 + (0 << 11) + (0 << 5);
        spi2.read(&spi2, c, d, 3);

        struct touchscreen_position page = {
                .x = d[0],
                .y = d[1],
                .z = d[2],
        };
