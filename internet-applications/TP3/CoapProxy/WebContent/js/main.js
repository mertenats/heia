...
var JAVA_PROJECT_NAME = 'TP3_CoapProxy';
...
function getWeatherStationData() {
  ...
  $.ajax( {
    ...
    success:function(data) {
      ...
      for (var deviceLocation in data.m_weatherStationMap) {
        // get a reference to the device
        var l_device = data.m_weatherStationMap[deviceLocation];
...
function updateDisplay(p_deviceLocation, p_device) {
  ...
  for (var deviceSensor in p_device.m_sensorList) {
    // get the reference of the sensor
    var sensor = p_device.m_sensorList[deviceSensor];

    output += ' <div class="col-xs-6 col-sm-4">';
    if (sensor.m_id == TEMP_SENSOR_ID) {
      output += '   <div class="circle '+ getColorFromTemperature(sensor.m_value) + '">';
    } else if (sensor.m_id == HUM_SENSOR_ID) {
      output += '   <div class="circle '+ getColorFromHumidity(sensor.m_value) + '">';
    } else if (sensor.m_id == PRES_SENSOR_ID) {
      output += '   <div class="circle '+ getColorFromPressure(sensor.m_value) + '">';
    } else {
    ...
    output += '         ' + sensor.m_value + '' + sensor.m_unit ;
    output += '       </div>';
    output += '     </div>';
    output += '   </div>';
    output += '   <p class="circle-title">' + sensor.m_id + '</p>';
    output += ' </div>';
  }
...
function updateDetailledPreviewWeatherStation(p_deviceLocation) {
  ...
  $.ajax( {
    ...
    success:function(data) {
      ...
      output += '<div class="row">';
      for (var deviceSensor in data.m_sensorList) {
        var sensor = data.m_sensorList[deviceSensor];
        output += ' <div class="col-xs-6 col-sm-4">';
      ...
      output += '<input type="hidden" id="hidden_state" name="hidden_state" value="' + data.m_state + '">';
